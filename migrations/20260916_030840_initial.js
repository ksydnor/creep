...
export async function up({ db }) {
  await db.run(sql`CREATE TABLE \`projects_tools\` (
  ...
  `)
  await db.run(sql`CREATE INDEX \`projects_tools_order_idx\` ON \`projects_tools\` (\`_order\`);`)
  await db.run(sql`CREATE INDEX \`projects_tools_parent_id_idx\` ON \`projects_tools\` (\`_parent_id\`);`)
  ...
  await db.run(sql`CREATE TABLE \`_projects_v_version_tools\` (
  	\`_order\` integer NOT NULL,
  	\`_parent_id\` integer NOT NULL,
  	\`id\` integer PRIMARY KEY NOT NULL,
  	\`tool\` text,
  ...
  `)
  await db.run(sql`CREATE INDEX \`_projects_v_version_tools_order_idx\` ON \`_projects_v_version_tools\` (\`_order\`);`)
  await db.run(sql`CREATE INDEX \`_projects_v_version_tools_parent_id_idx\` ON \`_projects_v_version_tools\` (\`_parent_id\`);`)
  ...
  await db.run(sql`CREATE INDEX \`_projects_v_version_sections_image_idx\` ON \`_projects_v_version_sections\` (\`image_id\`);`)
  ...
  await db.run(sql`CREATE TABLE \`_projects_v\` (
  	...
  	\`version_module_eyebrow\` text,
  	\`version_module_title\` text,
  	\`version_module_body\` text,
  	\`version_cover_image_id\` integer,
  	\`version_thumbnail_id\` integer,
  	\`version_tone\` text DEFAULT 'dark',
  	\`version_layout\` text DEFAULT 'grid',
  	\`version_hero_object_position\` text,
  	\`version_hero_image_transform\` text,
  ...
  `)
  await db.run(sql`CREATE INDEX \`site_settings_headshot_idx\` ON \`site_settings\` (\`headshot_id\`);`)
  await db.run(sql`CREATE INDEX \`site_settings_cv_idx\` ON \`site_settings\` (\`cv_id\`);`)
  await db.run(sql`CREATE INDEX \`site_settings_hero_image_idx\` ON \`site_settings\` (\`hero_image_id\`);`)
}

export async function down({ db }) {
  await db.run(sql`DROP TABLE \`projects_tools\`;`)
  await db.run(sql`DROP TABLE \`projects_sections\`;`)
  await db.run(sql`DROP TABLE \`projects_gallery\`;`)
  await db.run(sql`DROP TABLE \`projects\`;`)
  await db.run(sql`DROP TABLE \`_projects_v_version_tools\`;`)
