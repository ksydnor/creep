...
export function SiteHeader({ site, pages = [] }) {
  const pathname = usePathname();
  const navLinks = [
    { label: "Work", href: "/work" },
    { label: "About", href: "/about" },
    ...pages.map((page) => ({ label: page.title, href: `/${page.slug}` })),
  ...
  return (
    <header className="fixed inset-x-0 top-0 z-50 border-b border-white/10 bg-ink/75 backdrop-blur">
      <nav aria-label="Primary" className="flex items-center justify-between px-5 py-4 sm:px-8 lg:px-12">
        ...
        <div className="flex flex-wrap items-center justify-end gap-5 text-xs font-semibold uppercase tracking-exhibit text-bone/70">
          {navLinks.map((link) => (
            <Link
